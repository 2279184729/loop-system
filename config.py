...
# 任务超时（秒）
TASK_TIMEOUT = 3600
# 跳过 claude CLI 权限检查（环境变量 LOOP_SKIP_PERMISSIONS=1 启用）
DANGEROUSLY_SKIP_PERMISSIONS = os.environ.get("LOOP_SKIP_PERMISSIONS", "").lower() in ("1", "true", "yes")
...
class Colors:
    ...
    def supports_color():
        """检测终端是否支持颜色"""
        if Colors._color_support is not None:
            return Colors._color_support
        if os.name == 'nt':
            try:
                import ctypes
                kernel32 = ctypes.windll.kernel32
                kernel32.SetConsoleMode(kernel32.GetStdHandle(-11), 7)
                Colors._color_support = True
                return True
            except Exception:
                Colors._color_support = False
                return False
        Colors._color_support = hasattr(sys.stdout, 'isatty') and sys.stdout.isatty()
        return Colors._color_support

# ============================================================
# Claude API 系统提示词（用于构建 claude CLI prompt）
# ============================================================
ORCHESTRATOR_PLANNING_PROMPT = """你是一个资深技术架构师。分析用户的任务需求，直接生成详细的实施方案。

**重要：不要反问、不要提问、不要确认。直接输出 JSON。**

返回严格的 JSON 格式（用 ```json 代码块包裹）：
```json
{
